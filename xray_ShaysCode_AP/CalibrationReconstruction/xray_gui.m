...
%set(hObject, 'units','normalized','outerposition',[0 0 1 1]); AP this
%makes figure bigger but all the other things stay the same size :-(
...
%I1 = double(imadjust(imread([newFolder,filesep,'D1.tif']),[0 0.7]));
%I2 = double(imadjust(imread([newFolder,filesep,'D2.tif']),[0.3 1]));
...
if exist(calibrationFile,'file')
    ...
    guidata(hObject, handles);
    handles = plotFeatures(handles,line_corners);
    guidata(hObject, handles);
...
%recon_corners = [100,100; 100,400; 400,400; 400,100];
%recon_line_corners = [500,500; 900,900];
%array_controller('deleteControllers');
%array_controller('addController',handles.hDetector1Axes,recon_corners);
%array_controller('addController',handles.hDetector2Axes,recon_corners);
%array_controller('addController',handles.hDetector1Axes,recon_line_corners);
%array_controller('addController',handles.hDetector2Axes,recon_line_corners);
...
if strcmpi(get(handles.hMode,'String'),'Calibration Mode')
    display('Hiding array controller')
...
if ~isempty(handles.currentActiveFolder)
    ...
    if ~isempty(strctReconstruction.corners)
       ...
       [ x3d_reg, x3d_raw ]= reconstruct_register(strctReconstruction, handles.activeCalibration);
        save(reconstructionFile, 'strctReconstruction', 'x3d_reg', 'x3d_raw');
    end
end
...
function newhandles = plotFeatures(handles,line_corners)
try
    delete(handles.hImage1Annotation);
catch
end
...
if ~isempty(handles.Features{1})
    ...
    %  AP edit
%    set(handles.hDetector1Axes,'xlim', [min([handles.Features{1}(:,1); min(line_corners(:,1))]), max([handles.Features{1}(:,1); max(line_corners(:,1))])]+[-40 40]);
%    set(handles.hDetector1Axes,'ylim', [min([handles.Features{1}(:,2); min(line_corners(:,2))]), max([handles.Features{1}(:,2); max(line_corners(:,2))])]+[-40 40]);
...
%     AP edit
%    set(handles.hDetector2Axes,'xlim', [min([handles.Features{2}(:,1); min(line_corners(:,1))]), max([handles.Features{2}(:,1); max(line_corners(:,1))])]+[-40 40]);
%    set(handles.hDetector2Axes,'ylim', [min([handles.Features{2}(:,2); min(line_corners(:,2))]), max([handles.Features{2}(:,2); max(line_corners(:,2))])]+[-40 40]);
...
intensity2=interp2(handles.image1,p3(1) + t2 * linedir(1),p3(2) + t2 * linedir(2));
...
if fullAutomation || useUserDefinedRectsAndFeatures
    ...
    [Proj1,Proj2,rec_Err]=ComputeCameraMatricesFromFeaturesAndModel3(Features2,x3D,150,true);
    fprintf('Best error to model: %.2f um\n',rec_Err);
